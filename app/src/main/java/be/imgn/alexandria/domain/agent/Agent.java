...
/**
 * An LRM Agent: a person or a collective agent, held once and referred to by everything that involves it — a work it
 * authored, an expression it translated, an edition it published.
 *
 * <p>Aggregate root. Promoting it out of the works that mention it is what lets "Penguin", "Penguin Books" and "Penguin
 * Classics" be one thing with three names, and what lets Willa Muir be one record whether she is translating or
 * writing.
 *
...
public record Agent(AgentId id, AgentKind kind, String name, String sortName, Set<String> aliases) {
    ...
    /**
     * Aliases are stored sorted and free of anything that merely repeats the preferred name, so the file has no
     * redundant entries and no run-to-run ordering churn.
     */
