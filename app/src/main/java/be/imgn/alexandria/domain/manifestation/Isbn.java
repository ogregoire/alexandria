...
public final class Isbn {
    ...
    static String hyphenate13(String digits) {
        return digits.substring(0, 3) + "-" + digits.substring(3, 4) + "-"
                + digits.substring(4, 9) + "-" + digits.substring(9, 12) + "-" + digits.substring(12);
    }

    /**
     * The ISBN-10 equivalent of a 978-prefixed ISBN-13.
     *
     * <p>Needed because catalogues that predate 2007 — the BnF among them — index the older
     * form, so a search for 9782070360024 finds nothing while 2070360024 finds the record.
     * Returns empty for 979-prefixed ISBNs, which have no ISBN-10 equivalent at all.
     */
    public static java.util.Optional<String> toIsbn10(String isbn13) {
        ...
        return java.util.Optional.of(body + (check == 10 ? "X" : Integer.toString(check)));
    }

    /** The 978-prefixed ISBN-13 equivalent of an ISBN-10. */
    public static java.util.Optional<String> toIsbn13(String isbn10) {
        ...
        return java.util.Optional.of(body + ((10 - (sum % 10)) % 10));
    }
}
