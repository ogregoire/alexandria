...
/**
 * Jackson lives here and nowhere else. Every sealed hierarchy in the domain gets a
 * mix-in naming its variants, so the committed JSON carries a stable {@code "type"}
 * discriminator that reads well in a diff and does not shift when a record is renamed.
 *
 * <p>Derived accessors that happen to be getter-shaped are ignored here too, otherwise
 * Jackson would write them out and then refuse to read them back.
 */
final class Mixins {
    ...
    abstract static class AgentMixin {
        @JsonIgnore
        abstract java.util.stream.Stream<String> names();
    }
    ...
    abstract static class BibliographicDateMixin {
        @JsonIgnore
        abstract java.util.Optional<Integer> sortYear();
    }
    ...
    abstract static class AcquisitionMixin {
        ...
        @JsonIgnore
        abstract java.util.Optional<java.time.LocalDate> on();
    }
    ...
    static void applyTo(com.fasterxml.jackson.databind.ObjectMapper mapper) {
