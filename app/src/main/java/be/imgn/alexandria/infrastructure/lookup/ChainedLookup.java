 ...
 *
 * <p>First hit wins rather than merging across providers: one book's metadata coming from
 * one catalogue is easier to sanity-check than a blend of three, and the form is going to be
 * reviewed by hand anyway. The order is Open Library, then the BnF for the French
 * publishing it knows best, then Google Books.
 *
 * <p>A provider that throws is skipped rather than allowed to sink the whole lookup — an
 * outage at one service should not stop the others answering.
 */
public final class ChainedLookup implements BookLookup {
    ...
    public static ChainedLookup standard() {
        return standard(java.util.Optional.empty());
    }

    /**
     * @param contact an email or URL to identify this installation with. Supplying one
     *                triples the rate Open Library allows; without it every provider is
     *                held to its unidentified limit.
     */
    public static ChainedLookup standard(java.util.Optional<String> contact) {
        UserAgent caller = contact.map(UserAgent::identifiedBy).orElseGet(UserAgent::anonymous);
        return new ChainedLookup(List.of(
                new OpenLibraryLookup(caller),
                new BnfLookup(caller),
                new GoogleBooksLookup(caller)));
    }
