...
/**
 * Turns the three-letter language codes catalogues use into the two-letter ones the model
 * stores.
 *
 * <p>Library data is written in ISO 639-2/B, the <em>bibliographic</em> set, where French is
 * {@code fre} and German is {@code ger}. The JDK only knows 639-2/T, the terminological set,
 * where the same languages are {@code fra} and {@code deu}. Twenty languages differ between
 * the two, and those twenty are exactly the ones a European library catalogue is full of, so
...
final class Iso639 {
    ...
    private static final Map<String, String> BIBLIOGRAPHIC_TO_TERMINOLOGIC = Map.ofEntries(
            Map.entry("alb", "sqi"), Map.entry("arm", "hye"), Map.entry("baq", "eus"),
            Map.entry("bur", "mya"), Map.entry("chi", "zho"), Map.entry("cze", "ces"),
            Map.entry("dut", "nld"), Map.entry("fre", "fra"), Map.entry("geo", "kat"),
            Map.entry("ger", "deu"), Map.entry("gre", "ell"), Map.entry("ice", "isl"),
            Map.entry("mac", "mkd"), Map.entry("mao", "mri"), Map.entry("may", "msa"),
            Map.entry("per", "fas"), Map.entry("rum", "ron"), Map.entry("slo", "slk"),
            Map.entry("tib", "bod"), Map.entry("wel", "cym"));
